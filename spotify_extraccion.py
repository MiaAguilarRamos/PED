...
def iniciar_extraccion():
    url = "https://mx.investing.com/equities/spotify-technology-balance-sheet"
    cabeceras_http = {
    ...
    if respuesta.status_code == 200:
        ...
        # Visualiza algunas filas para entender la estructura
        for fila in filas[:5]:
            print(fila)

        # Filtra filas que tienen el mismo número de columnas que las cabeceras
        filas_validas = [fila for fila in filas if len(fila) == len(cabeceras)]

        # Crea un DataFrame
        df = pd.DataFrame(filas_validas, columns=cabeceras)
        print(df.sample(10))
        df.to_csv("datasets/spotify_balance.csv", index=False)
    else:
